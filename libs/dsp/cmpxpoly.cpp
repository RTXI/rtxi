...
CmplxPolynomial::CmplxPolynomial(const complex coeff_1, const complex coeff_0)
{
  Degree = 1;
  Coeff = new complex[2];
  RemCoeff = NULL;
...
CmplxPolynomial::CmplxPolynomial(const complex* coeff, const int degree)
{
  Degree = degree;
  Coeff = new complex[degree + 1];
  RemCoeff = NULL;
...
CmplxPolynomial::CmplxPolynomial(const double* coeff, const int degree)
{
  Degree = degree;
  Coeff = new complex[degree + 1];
  RemCoeff = NULL;
...
CmplxPolynomial::FindRoots(void)
{
  ...
  for (i = 0; i < Degree - 1; i++) {
    ...
    status =
      LaguerreMethod(&work_poly, &(Root[i]), epsilon, epsilon2, max_iter);
    if (status < 0) {
      std::cout << "Laguerre method did not converge" << std::endl;
      exit(55);
    }
    std::cout << "Root[" << i << "] = " << Root[i] << " (" << status << ")"
              << std::endl;
  ...
  for (i = 0; i < Degree; i++) {
    ...
    std::cout << "Polished Root[" << i << "] = " << Root[i] << " (" << status
              << ")" << std::endl;
    pause();
  }
  return;
...
void
CmplxPolynomial::ReflectRoot(int root_idx)
{
...
void
CmplxPolynomial::BuildFromRoots(void)
{
...
void
CmplxPolynomial::DumpToStream(std::ostream* output_stream)
{
  (*output_stream) << "Degree = " << Degree << std::endl;

  for (int i = Degree; i >= 0; i--) {
    (*output_stream) << "Coeff[" << i << "] = " << Coeff[i] << std::endl;
  }
  return;
}
...
int
CmplxPolynomial::GetDegree(void)
{
  return (Degree);
}
...
complex
CmplxPolynomial::GetCoeff(int k)
{
  return Coeff[k];
}
...
void
CmplxPolynomial::CopyCoeffs(complex* coeff)
{
  for (int i = 0; i <= Degree; i++)
    coeff[i] = Coeff[i];
  return;
