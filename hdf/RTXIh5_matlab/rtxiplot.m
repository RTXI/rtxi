...
pstr = ['[{''',parameters.modelName{1},' : ',parameters.parameterName{1},' : ',num2str(parameters.values{1}),'''}'];
for p=2:length(parameters.parameterName)
    pstr = [pstr,';{''',strrep(parameters.modelName{p},'''',''''''),' : ',strrep(parameters.parameterName{p},'''',''''''),' : ',num2str(parameters.values{p}),'''}'];
end
pstr = [pstr,']'];
