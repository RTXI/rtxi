...
ComediDevice::ComediDevice(void *d,ComediLib::comedi_t *cd,std::string name,IO::channel_t *chan,size_t size)
    : DAQ::Device(name,chan,size), device(d), comedi_device(cd) {
    ...
    if( (ComediLib::comedi_get_subdevice_flags(comedi_device,aiSubDevice) & SDF_SOFT_CALIBRATED) > 0
            ...
            char *calibpath = comedi_get_default_calibration_path(comedi_device);
            calibration = ComediLib::comedi_parse_calibration_file( calibpath ); // Returns NULL if failed
            free( calibpath );
            if( calibration == NULL )
...
std::string ComediDevice::getAnalogRangeString(type_t type,index_t channel,index_t index) const
{
    ...
    comedi_krange range;
    comedi_get_krange(device,subdevice[type].id,channel,index,&range);
...
int ComediDevice::setAnalogCalibration(type_t type,index_t channel) {
    ...
    ComediLib::comedi_polynomial_t polynomial;
    int retval = -1;
    ...
    if( calibration != NULL ) { // Board was soft calibrated
        ...
        if( type == AI ) 
            ...
            retval = ComediLib::comedi_get_softcal_converter(subdevice[type].id,channel,chanPtr->range,
                                                             ComediLib::COMEDI_FROM_PHYSICAL,calibration,&polynomial);
        else
        ...
        if( type == AI )
            ...
            retval = ComediLib::comedi_get_hardcal_converter(comedi_device,subdevice[type].id,channel,chanPtr->range,
                                                             ComediLib::COMEDI_FROM_PHYSICAL,&polynomial);
        else
            ERROR_MSG("ComediDevice::setAnalogCalibration : invalid type\n");
