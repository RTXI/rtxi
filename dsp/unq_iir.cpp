...
UnquantDirectFormIir::UnquantDirectFormIir(int num_numer_coeff,
                                           int num_denom_coeff,
                                           double* numer_coeff,
                                           double* denom_coeff)
{
...
double
UnquantDirectFormIir::ProcessSample(double input_val)
{
  double output_val;
  double term, sum;
  ...
  for (tap_indx = 1; tap_indx <= Num_Denom_Coeff; tap_indx++) {
    term = Denom_Coeff[tap_indx] * Output_Buffer[output_read_indx];
    sum += term;
    output_read_indx--;
...
int
UnquantDirectFormIir::GetNumNumerCoeff(void)
{
  return (Num_Numer_Coeff);
}

int
UnquantDirectFormIir::GetNumTaps(void)
{
  return (Num_Numer_Coeff);
}

int
UnquantDirectFormIir::GetNumDenomCoeff(void)
{
