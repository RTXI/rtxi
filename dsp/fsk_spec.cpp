  ...
  for (freq_idx = 0; freq_idx < 128; freq_idx++) {
    ...
    for (n = 1; n <= big_m; n++) {
      ...
      for (m = 1; m <= big_m; m++) {
        ...
        b_mn = (cos(TWO_PI * f * big_t - alpha_mn) - psi * cos(alpha_mn)) /
               (1.0 + psi * psi - 2 * psi * cos(TWO_PI * f * big_t));
        sum2 += b_mn * a_n * a_m;
      } // end of loop over m
    }   // end of loop over n
    phi = big_t * (sum / 2.0 + (sum2 / big_m)) / big_m;
    if (freq_idx == 0)
      std::cout << "at f=0, phi = " << phi << std::endl;
  } // end of loop over freq_idx
}
double
CpfskSpectrum::GetPsdValue(double freq)
{
  ...
  for (n = 1; n <= Big_M; n++) {
    ...
    for (m = 1; m <= Big_M; m++) {
      ...
      b_mn = (cos(TWO_PI * freq * Big_T - alpha_mn) - psi * cos(alpha_mn)) /
             (1.0 + psi * psi - 2 * psi * cos(TWO_PI * freq * Big_T));
      sum2 += b_mn * a_n * a_m;
    } // end of loop over m
  }   // end of loop over n
  phi = Big_T * (sum / 2.0 + (sum2 / Big_M)) / Big_M;
