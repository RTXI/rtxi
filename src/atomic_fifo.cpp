...
bool AtomicFifo::write(const void* buffer,
                       size_t itemSize)  // It is an absolute requirement only
                                         // one thread calls write
{
  ...
  if (itemSize >= fifoSize
          - ((fifoSize + current_tail - head.load(std::memory_order_seq_cst))
             % fifoSize))
  {
