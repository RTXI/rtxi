...
Settings::Object::State Settings::Object::State::loadState(
...
void Settings::Manager::foreachObject(void (*callback)(Object*, void*),
                                      void* param)
{
  Mutex::Locker lock(&mutex);
...
int Settings::Manager::load(const std::string& filename)
{
  ...
  if (doesnotexist) {
    if (num_settings == 10) {
      userprefs.setValue(
          "/recentSettingsList/" + QString::number(oldestsetting),
      ...
      userprefs.setValue(
          "/recentSettingsList/" + QString::number(num_settings++),
          QVariant(QString::fromStdString(filename)));
...
void Settings::Manager::acquireID(Settings::Object* object,
                                  Settings::Object::ID id)
{
  ...
  if (currentID == Object::INVALID) {
    ERROR_MSG(
        "Settings::Manager::acquireID : maximum number of settings objects "
        "loaded.\n");
