...
IncrementalPlot::IncrementalPlot(QWidget *parent) : BasicPlot(parent), d_data(NULL), d_curve(NULL), l_data(NULL), l_curve(NULL) {
	setAutoReplot(false);
}
...
void IncrementalPlot::appendData(double x, double y, QwtSymbol::Style s) {
	appendData(&x, &y, 1, s);
}
...
void IncrementalPlot::appendData(double *x, double *y, int size, QwtSymbol::Style s) {
	if (d_data == NULL) d_data = new CurveData;
...
void IncrementalPlot::appendLine(double *x, double *y, int size) {
	if (l_data == NULL) l_data = new CurveData;
